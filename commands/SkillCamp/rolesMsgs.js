...
module.exports = {
	...
	async execute(interaction) {
		...
		let embed, row, row2, row3, row4, row5;
		if (guildName == 'CodeCamp') {
			// PROD remove return;
			return;
		} else if (guildName == 'CampMaster') {
			// get channel
			const channel = guild.channels.cache.get(guildData.SkillCamps.CampMaster.channels.tagYourself.channelID);
			...
			const regMsg = await channel.send({ embeds: [embed], components: [row, row2, row3, row4, row5] });
